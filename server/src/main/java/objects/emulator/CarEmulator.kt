...
class CarEmulator(uid: Int, val testRoom: EmulatedRoom, val useRandom: Boolean, val randomGenerator: SeedRandomGenerator) : Car(uid) {

    private var xReal = 0.0
    ...
    override fun scan(angles: IntArray, attempts: Int, windowSize: Int, smoothing: SonarRequest.Smoothing): ListenableFuture<Response> {
        ...
        angles.forEach { angle ->
            val angleFinal = getSensorAngleWithOX(angle, carAngle.degs())
            val xSensor1: Double
            val ySensor1: Double
            //tg can be equal to inf if angle = 90 or 270. its vertical line. x1 = x0, y1 = y0+-[any number. eg 1]
            when (angleFinal) {
                90 -> {
                    xSensor1 = xSensor0
    ...
    private fun getSensorAngleWithOX(requestAngle: Int, carAngle: Int): Int {
